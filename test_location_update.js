...
async function run() {
    try {
        console.log('Sending location update at STAGE restaurant...');
        const locRes = await fetch('https://rally-staging-9ae8.up.railway.app/api/v1/riders/location', {
            method: 'POST',
            headers: {
        ...
        console.log('Location update status:', locRes.status, await locRes.text());
